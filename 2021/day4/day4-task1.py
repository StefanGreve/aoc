...
def main(path: str):
    ...
    for turn in range(len(numbers)):
        for board in boards:
            board.numbers = numbers[0:turn]
            for i in range(1, board.dim+1):
                board.on_row = set(board.row(i)).issubset(set(board.numbers))
                board.on_col = set(board.col(i)).issubset(set(board.numbers))
                if board.bingo:
                    return board

if __name__ == '__main__':
    result = main('input')
    print(f"The final score is {result.score}. The the winning board was:\n")
    print(result)
